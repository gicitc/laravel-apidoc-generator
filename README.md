...
```sh
composer require gicitc/laravel-apidoc-generator
```
...
```bash
php artisan vendor:publish --provider="Gicitc\ApiDoc\ApiDocGeneratorServiceProvider" --tag=apidoc-config
```
...
```php
$app->register(\Gicitc\ApiDoc\ApiDocGeneratorServiceProvider::class);
```
